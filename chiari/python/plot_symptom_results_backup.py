...
data_list = [symptom_name_data,tuple_data,alg_data,value_data]
print("DL")
print(data_list)
data_array = array(data_list,dtype='object').T
data_df = pd.DataFrame(data_array,columns=['Symptom','Best Tuple','Algorithm','Value (sens*spec)'])
#Convert DF to latex
...
ind=arange(len(value_data))
width=0.5
color=(0.2588,0.4433,1.0) #Light blue
bar(ind,value_data,width,color=color)
xticks(ind+width,symptom_name_data,rotation=45,ha='right')

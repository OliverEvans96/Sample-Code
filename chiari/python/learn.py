...
for size_index,tuple_size in enumerate(size_list):
    ...
    #Determine when to update status bar (every 5%)
    status_bar_marks=[n_tuples//20*i for i in range(20)]
    ...
    for tuple_num,feature_numbers in enumerate(size_tuples):
        ...
        #Show progress bar for this tuple size every 5%
        for i,mark in enumerate(status_bar_marks):
            if(tuple_num==mark):
                print('\r',end='')
                print("[%-20s] %d%%" % ('='*(i-1)+'>'*(i>0),5*i),end='')
                #stdout.flush()
